...
input = DATA.each_line.map do |line|
  ...
  [(range_start..range_end), char.strip.chomp(":"), password.strip]
end

puts(input.count do |range, char, password|
  range.cover?(password.count(char))
end)

puts(input.count do |range, char, password|
  (password[range.first - 1].eql?(char)) != (password[range.last - 1].eql?(char))
end)
