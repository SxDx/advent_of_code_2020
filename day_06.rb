...
input = DATA.each_line(chomp: true).slice_before(&:empty?).map do |group|
  group.reject(&:empty?)
end

puts input.map { |group| group.join.scan(/\w/).uniq.join.length }.sum

puts(input.map do |group|
  group.join.scan(/\w/).tally.select { |_k, v| v.eql?(group.size) }.keys.length
end.sum)
