...
def valid_height(hgt)
  if hgt&.end_with?("cm")
    (150..193).cover?(hgt[0..-2]&.to_i)
  elsif hgt&.end_with?("in")
    (59..76).cover?(hgt[0..-2]&.to_i)
  else
    false
  end
end
...
puts(passports.count do |p|
  p.delete(:cid)
  checks = [
    (1920..2002).cover?(p.dig(:byr)&.to_i),
    (2010..2020).cover?(p.dig(:iyr)&.to_i),
    (2020..2030).cover?(p.dig(:eyr)&.to_i),
    valid_height(p.dig(:hgt)),
    p.dig(:hcl)&.match(/^#[0-9a-f]{6}$/) ? true : false,
    EYE_COLORS.include?(p.dig(:ecl)),
    p.dig(:pid)&.match(/^\d{9}$/) ? true : false
  ]
